...
public class LocalDateTimeField extends CustomField<LocalDateTime> {
	 ...
	 */
	public int getHours() {
		final LocalDateTime d = getValue();
		return d.getHour();
	}
	...
	private NativeSelect getSelect() {
		...
		select.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(
				...
				updateValue();
				LocalDateTimeField.this.fireValueChange(true);
	...
	private void updateValue() {
		...
		newDate.withSecond(0);
		newDate.withMinute(0);
		newDate.withHour(0);
	...
	private void updateFields() {
		...
		final LocalDateTime val = getValue();
		...
		// check hour bounds
		if (val.getHour() < minHours) {
			// guess
			int compatibleVal = val.getHour();
		...
		hourSelect.setValue(val.getHour());

		// check minute interval
		if (val.getMinute() % intervalMinutes != 0) {
			// guess
			int compatibleVal = val.getMinute();
			while (compatibleVal > 0) {
			...
			val.withMinute(compatibleVal);
		}
		minuteSelect.setValue(val.getMinute());

		secondSelect.setValue(val.getSecond());
